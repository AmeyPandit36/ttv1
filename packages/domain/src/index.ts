...
export function validateTimetable(input:SolverInput, assignments:Assignment[]): ValidationReport {
  ...
  for(const s of input.sessions){
    const a=bySession.get(s.id); if(!a){conflicts.push({code:'MISSING_SESSION',message:`${s.title} is not scheduled`,sessionIds:[s.id]});continue;}
    const r=resMap.get(a.resourceId); if(!r){conflicts.push({code:'UNKNOWN_RESOURCE',message:`Unknown resource ${a.resourceId}`,sessionIds:[s.id]});continue;}
    const eligible=resourceEligible(s,r); for(const reason of eligible.reasons) conflicts.push({code:'RESOURCE_INELIGIBLE',message:`${s.title}: ${reason}`,sessionIds:[s.id],resourceId:r.id});
    if(a.slotIds.length!==s.duration) conflicts.push({code:'INVALID_DURATION',message:`${s.title} requires ${s.duration} continuous periods`,sessionIds:[s.id],slotIds:a.slotIds});
    const actual=a.slotIds.map(x=>slotMap.get(x)).filter(Boolean) as Slot[]; if(actual.length===s.duration && actual.some((x,i)=>i>0&&(x.dayId!==actual[0].dayId||x.index!==actual[i-1].index+1))) conflicts.push({code:'NON_CONTIGUOUS',message:`${s.title} spans a break or non-contiguous period`,sessionIds:[s.id],slotIds:a.slotIds});
    ...
    for(const p of input.policies.filter(p=>p.active&&p.strength==='HARD')){const scope=p.scope as Record<string,unknown>,params=p.parameters as Record<string,unknown>;const applies=(!scope.departmentId||scope.departmentId===s.preferredDepartmentId)&&(!scope.facultyId||scope.facultyId===s.facultyId)&&(!scope.sessionType||scope.sessionType===s.sessionType);if(!applies)continue;if(p.type==='RESOURCE_PROHIBITION'&&Array.isArray(params.resourceIds)&&params.resourceIds.includes(r.id))conflicts.push({code:'POLICY_VIOLATION',message:`${s.title} uses ${r.name}, prohibited by policy ${p.name}`,sessionIds:[s.id],resourceId:r.id,evidence:{policyId:p.id}});if(p.type==='TIME_RESTRICTION'&&Array.isArray(params.prohibitedSlotIds)&&a.slotIds.some(id=>(params.prohibitedSlotIds as unknown[]).includes(id)))conflicts.push({code:'POLICY_VIOLATION',message:`${s.title} occupies a period prohibited by policy ${p.name}`,sessionIds:[s.id],slotIds:a.slotIds,evidence:{policyId:p.id}});}
  }
