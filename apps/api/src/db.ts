...
async function hydrate() {
    ...
    faculty.splice(0, faculty.length, ...fs.map(f => ({ ...f, unavailableSlotIds: fas.filter(a => a.facultyId === f.id && a.kind === 'UNAVAILABLE').map(a => a.timeSlotId), preferredSlotIds: fas.filter(a => a.facultyId === f.id && a.kind === 'PREFERRED').map(a => a.timeSlotId) })));
    ...
    resources.splice(0, resources.length, ...rs.map(r => ({ ...r, capabilities: rc.filter(c => c.resourceId === r.id).map(c => c.name), unavailableSlotIds: ras.filter(a => a.resourceId === r.id).map(a => a.timeSlotId) })));
